...
from openpi.serving.websocket_policy_server import WebsocketPolicyServer, resolve_model_path
...
def _multi():
    ...
    return WebsocketPolicyServer(
        policies=policies, default="exp_v14/70000", labels={"exp_v14/70000": "v14"}
    )
...
def test_connection_policy_routes_and_rejects():
    server = _multi()
    mid, policy = server._connection_policy("/m/exp_v13/70000")
    assert mid == "exp_v13/70000" and policy is server._policies[mid]
    assert server._connection_policy("/") == ("exp_v14/70000", server._policies["exp_v14/70000"])
    assert server._connection_policy("/m/gone/1") == (None, None)


def test_single_policy_accepts_any_path():
    policy = _StubPolicy()
    server = WebsocketPolicyServer(policy=policy)
    assert server._connection_policy("/whatever") == (None, policy)
